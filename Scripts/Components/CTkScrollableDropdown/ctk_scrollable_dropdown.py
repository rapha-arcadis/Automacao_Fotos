...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
        ...
        self.no_match = customtkinter.CTkLabel(self.frame, text="Não encontrado")
        self.height = height
    ...
    def _init_buttons(self, **button_kwargs):
        ...
        # Para um width=350, geralmente 45 a 50 caracteres cabem perfeitamente.
        limite_letras = 32

        for row in self.values:
            # Se o texto for maior que o limite, corta e adiciona as reticências
            texto_visivel = row if len(row) <= limite_letras else row[:limite_letras] + "..."
            
            self.widgets[self.i] = customtkinter.CTkButton(self.frame,
                                                          text=texto_visivel,  # Mostra o texto cortado com "..."
                                                          ...
                                                          command=lambda k=row: self._attach_key_press(k), **button_kwargs) # Envia o texto original completo
    ...
    def live_update(self, string=None):
        ...
        self._deiconify()
        i = 1
        max_limit = 50 # Limite: só renderiza 50 itens por vez
        
        # Garante que a string não seja None e converte para minúsculo
        busca = string.lower() if string else ""

        # Separa o que o usuário digitou em uma lista de palavras. 
        # Ex: "bosch campinas" vira ["bosch", "campinas"]
        palavras_busca = busca.split()
        
        for key in self.widgets.keys():
            s = self.values[key].lower()

            # Se o campo estiver vazio, mostra tudo.
            if not busca:
                similar = True
            else:
                # Se o campo estiver vazio (busca == ""), mostra tudo (até o limite). 
                # Se tiver texto, verifica se o texto está contido no nome.
                similar = all(palavra in s for palavra in palavras_busca)
            
            # Se não bateu com a busca, ou se já atingimos 50 itens na tela, esconde o botão.
            if not similar or i > max_limit:
